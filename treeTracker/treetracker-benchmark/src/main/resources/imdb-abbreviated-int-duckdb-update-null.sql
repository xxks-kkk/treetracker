/**
  ...
  instruction: ./duckdb /home/zeyuanhu/projects/treetracker2/main.duckdb < /home/zeyuanhu/projects/treetracker2/treeTracker/treetracker-benchmark/src/main/resources/imdb-abbreviated-int-duckdb-update-null.sql
 */

BEGIN TRANSACTION;

UPDATE imdb_int.aka_name SET
    person_id = CASE WHEN person_id is NULL THEN 0
                     ELSE person_id + 1 END;
UPDATE imdb_int.aka_title SET
    movie_id = CASE WHEN movie_id is NULL THEN 0
                    ELSE movie_id + 1 END;
UPDATE imdb_int.cast_info SET
                          person_id = CASE WHEN person_id is NULL THEN 0
                                           ELSE person_id + 1 END,
                          movie_id = CASE WHEN movie_id is NULL THEN 0
                                          ELSE movie_id + 1 END,
...
UPDATE imdb_int.movie_companies SET
                                ...
                                company_type_id = CASE WHEN company_type_id is NULL THEN 0
                                                       ELSE company_type_id + 1 END;
UPDATE imdb_int.movie_info_idx SET
                               movie_id = CASE WHEN movie_id is NULL THEN 0
                                               ELSE movie_id + 1 END,
                               info_type_id = CASE WHEN info_type_id is NULL THEN 0
                                                   ELSE info_type_id + 1 END;
UPDATE imdb_int.movie_keyword SET
                              movie_id = CASE WHEN movie_id is NULL THEN 0
                                              ELSE movie_id + 1 END,
                              keyword_id = CASE WHEN keyword_id is NULL THEN 0
                                                ELSE keyword_id + 1 END;
UPDATE imdb_int.movie_link SET
                           movie_id = CASE WHEN movie_id is NULL THEN 0
                                           ELSE movie_id + 1 END,
                           ...
                           link_type_id = CASE WHEN link_type_id is NULL THEN 0
                                               ELSE link_type_id + 1 END;
UPDATE imdb_int.name SET
    person_id = CASE WHEN person_id is NULL THEN 0
                     ELSE person_id + 1 END;
UPDATE imdb_int.role_type SET
    role_id = CASE WHEN role_id is NULL THEN 0
                   ELSE role_id + 1 END;
UPDATE imdb_int.title SET
                      movie_id = CASE WHEN movie_id is NULL THEN 0
                                      ELSE movie_id + 1 END,
                      kind_id = CASE WHEN kind_id is NULL THEN 0
                                     ELSE kind_id + 1 END;
UPDATE imdb_int.movie_info SET
                           movie_id = CASE WHEN movie_id is NULL THEN 0
                                           ELSE movie_id + 1 END,
                           info_type_id = CASE WHEN info_type_id is NULL THEN 0
                                               ELSE info_type_id + 1 END;
UPDATE imdb_int.person_info SET
                            person_id = CASE WHEN person_id is NULL THEN 0
                                             ELSE person_id + 1 END,
                            info_type_id = CASE WHEN info_type_id is NULL THEN 0
                                                ELSE info_type_id + 1 END;
