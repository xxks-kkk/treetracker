...
DROP VIEW IF EXISTS imdb.q19a_cast_info CASCADE;
DROP VIEW IF EXISTS imdb.q19a_company_name CASCADE;
DROP VIEW IF EXISTS imdb.q19a_info_type CASCADE;
DROP VIEW IF EXISTS imdb.q19a_movie_companies CASCADE;
DROP VIEW IF EXISTS imdb.q19a_movie_info CASCADE;
DROP VIEW IF EXISTS imdb.q19a_name CASCADE;
DROP VIEW IF EXISTS imdb.q19a_role_type CASCADE;
DROP VIEW IF EXISTS imdb.q19a_title CASCADE;

CREATE VIEW imdb.q19a_cast_info as
SELECT person_id, movie_id, person_role_id, role_id
FROM imdb.cast_info as ci
WHERE ci.note IN ('(voice)', '(voice: Japanese version)', '(voice) (uncredited)', '(voice: English version)');

CREATE VIEW imdb.q19a_company_name as
SELECT company_id
FROM imdb.company_name as cn
WHERE cn.country_code = '[us]';

CREATE VIEW imdb.q19a_info_type as
SELECT info_type_id
FROM imdb.info_type as it
WHERE it.info = 'release dates';

CREATE VIEW imdb.q19a_movie_companies as
...
CREATE VIEW imdb.q19a_movie_info as
...
CREATE VIEW imdb.q19a_name as
SELECT person_id
FROM imdb.name as n
WHERE n.gender ='f' AND n.name LIKE '%Ang%';

CREATE VIEW imdb.q19a_role_type as
SELECT role_id
FROM imdb.role_type as rt
WHERE rt.role ='actress';

CREATE VIEW imdb.q19a_title as
