...
public class BenchmarkMicroBenchQueries
{
    @State(Scope.Benchmark)
    public static class BenchState
    {
        @Param({"TTJHP", "TTJHP_NO_NG", "HASH_JOIN"})
        public JoinOperator joinOperator;
        @SuppressWarnings("checkstyle:AnnotationUseStyle")
        @Param({"TwoWayJoinQuery1"})
        public MicroBenchQueries microBenchQueries;
        JoinFragmentType query;

        @Setup(Level.Iteration)
        public void setUp()
        {
            query = queryProvider(joinOperator, microBenchQueries, duckDBJdbcClientSupplier.get());
        }
    ...
    {
        benchmark(BenchmarkMicroBenchQueries.class, MICROBENCH_QUERIES_RESULT_STORED_PATH).run();
    }
}
