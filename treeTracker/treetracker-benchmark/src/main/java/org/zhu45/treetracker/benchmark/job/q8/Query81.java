...
public class Query81
        extends Query
{
    public Query81(JoinFragmentContext context)
    {
        super(context);
    }
    ...
    protected Pair<Plan, List<Operator>> constructQuery()
    {
        ...
        MultiwayJoinOrderedGraph orderedGraph = getMultiwayJoinOrderedGraph(Arrays.asList(
                ...
                asEdge(movieCompaniesNode, titleNode)), akaNameNode);

        Pair<Plan, List<Operator>> pair = createFixedPhysicalPlanFromQueryGraph(orderedGraph);
        Plan plan = pair.getKey();

        LinkedList<SchemaTableName> expectedSchemaTableNames = new LinkedList<>(
                Arrays.asList(akaNameNode.getSchemaTableName(),
