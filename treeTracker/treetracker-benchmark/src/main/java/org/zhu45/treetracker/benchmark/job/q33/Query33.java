...
public class Query33
        extends Query
{
    public Query33(JoinFragmentContext context)
    {
        super(context);
    }
    ...
    protected Pair<Plan, List<Operator>> constructQuery()
    {
        ...
        MultiwayJoinOrderedGraph orderedGraph = getMultiwayJoinOrderedGraph(Arrays.asList(
                asEdge(movieLinkNode, titleNode),
                asEdge(movieLinkNode, linkTypeNode),
                asEdge(titleNode, movieInfoIdxNode),
                asEdge(titleNode, kindTypeNode),
                asEdge(movieInfoIdxNode, infoTypeNode),
                asEdge(movieInfoIdxNode, movieCompaniesNode),
                asEdge(movieCompaniesNode, companyNameNode)), movieLinkNode);
