...
public class JOBQueryPerformance
{
    private static final LoggerProvider.TreeTrackerLogger log;

    static {
        if (Switches.DEBUG) {
            log = getLogger(JOBQueryPerformance.class);
        }
        else {
            log = null;
        }
    ...
    private static void run(JoinOperator algorithm, Class<? extends Query> queryClazz, boolean enableRules)
    {
        ...
        try {
            System.out.println("result set size: " + query.evalWithResultSize());
        }
        finally {
    ...
    {
        run(JoinOperator.TTJHP, Query10aFindOptJoinTree.class, true);
    }
}
