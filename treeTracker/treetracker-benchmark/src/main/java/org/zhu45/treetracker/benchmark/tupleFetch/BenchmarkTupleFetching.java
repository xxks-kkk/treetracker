...
public class BenchmarkTupleFetching
{
    @State(Scope.Benchmark)
    public static class BenchState
    {
        ...
        @SuppressWarnings("checkstyle:AnnotationUseStyle")
        @Param({"Q8cOptJoinTreeOptOrdering"})
        public TupleFetchQueries tupleFetchQueries;
        JoinFragmentType query;

        @Setup(Level.Iteration)
        public void setUp()
        {
            query = queryProvider(joinOperator, tupleFetchQueries, duckDBJdbcClientSupplier.get());
        }
    ...
    @State(Scope.Benchmark)
    public static class BenchStateWithPopulateDomain
    {
        @SuppressWarnings("checkstyle:AnnotationUseStyle")
        @Param({"HASH_JOIN"})
        public JoinOperator joinOperator;
    ...
    @Benchmark
    public void benchWithPopulateDomain(BenchState state)
    {
        state.query.eval();
    }
    ...
    {
        benchmark(BenchmarkTupleFetching.class, OTHERS_RESULT_STORED_PATH).run();
    }
}
