...
public class TestTTJAggregateStatisticsInformation
{
            ...
            implements TestCases
    {
        ...
        public Triple<Plan, List<Operator>, Integer> physicalPlanForTestNumberOfHashTableProbe()
        {
            ...
            // To sum up, 4 + 5 + 2 = 11.
            return Triple.of(testCase.getLeft(), testCase.getMiddle(), 11);
        }
    ...
    public static class TestNumberOfDeletionPropagationTriggeredCases
            implements TestCases
    {
        private String schemaName;
        private JdbcClient jdbcClient;
        ...
        public TestNumberOfDeletionPropagationTriggeredCases(TestingPhysicalPlanBase base)
        {
            this.schemaName = base.getDatabase().getSchemaName();
            this.jdbcClient = base.getDatabase().getJdbcClient();
            this.base = base;
        ...
        public Triple<Plan, List<Operator>, Integer> physicalPlanForTestNumberOfHashTableProbe()
        {
            ...
            // deletion propagation triggered in the form of initiating PassContext() calls whenever join failure happens.
            // For example, once all R(2,3) are removed due to join failure
            // with B, additional deleteDT() will be triggered to remove C(3) by adding it to no-good list.
            return Triple.of(testCase.getLeft(), testCase.getMiddle(), 3);
        }
        ...
        public Triple<Plan, List<Operator>, Integer> physicalPlanForTestNumberOfHashTableProbe2()
        {
            ...
            // The next tuple to try is (red,3,2), which doesn't need hash probe due to it is already in MatchingTuples.
            // (red,3,2) joins with B(2) and R(3,2) and form the join result. In other words, in this case, "deletion propagation"
            // has not introduced additional number of hash table probes. Thus, the number of deletion propagation initiated should be 0.
            return Triple.of(pair.getLeft(), pair.getRight(), 0);
        }
    }

    private Object[][] testNumberOfDeletionPropagationTriggeredDataProvider()
    {
        return twoDlistTo2DArray(buildSpecificTestCases(base, List.of(TestNumberOfDeletionPropagationTriggeredCases.class)));
    }

    @ParameterizedTest
    @MethodSource("testNumberOfDeletionPropagationTriggeredDataProvider")
    public void testNumberOfDeletionPropagationTriggered(Triple<Plan, List<Operator>, Integer> triple)
    {
        int expectedValue = triple.getRight();
        if (traceLogger.isDebugEnabled()) {
            traceLogger.debug("expectedValue:\n" + expectedValue);
        ...
        TTJAggregateStatisticsInformation aggregateStatisticsInformation = testStatistics(triple);
        assertEquals(expectedValue, aggregateStatisticsInformation.getNumberOfDeletionPropagationTriggered());
    }
