"""
Like job_with_predicates.py but only focus on HJ vs. TTJ
"""
...
from plot.constants import DATA_SOURCE_CSV, HJ, TTJ, FIG_SAVE_LOCATION, PLOT_FUNCTION, ALGORITHMS_TO_PLOT, \
    PERCENTILES, COLUMN_RIGHT_BOUND, ENABLE_NORMALIZATION
...
def job_perf_improvements(plot_conf: dict):
    def bar_plot(ax, data, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        for i, (name, values) in enumerate(data.items()):
            ...
            bars.append(bar[0])
        if plot_conf[ENABLE_NORMALIZATION]:
            legend_loc = (0.7,0.8)
        else:
            legend_loc = (0.7,0.7)
        if legend:
            ax.legend(bars, [r'$\mathsf{HJ}$', r'$\mathsf{TTJ}$'], frameon=False, fontsize=20, ncol=3,
                      loc=legend_loc)
    ...
    if plot_conf[ENABLE_NORMALIZATION]:
        ...
        for algorithm, dps in grouped_data.items():
            hj = grouped_data[HJ]
            data_normalized[algorithm] = [round(algorithm_time / hj_time, 1) for hj_time, algorithm_time in zip(hj, grouped_data[algorithm])]
        grouped_data = data_normalized

    matplotlib.rcParams['mathtext.fontset'] = 'stix'
    fig, ax = plt.subplots(figsize=(14, 3), dpi=140)
    font = {'family': 'Helvetica',
            'size': 5}
    bar_plot(ax, grouped_data, colors=['#00994D', '#FF9933'], total_width=.7, single_width=1,
             fontdict=font)
    font2 = {'family': 'Helvetica',
             'weight': 'bold',
             'size': 15}
    if plot_conf[ENABLE_NORMALIZATION]:
        plt.ylabel('Normalized Runtime', fontdict=font2)
    else:
        plt.ylabel('Runtime', fontdict=font2)
    plt.xticks(rotation=270)
    plt.tight_layout()
    filename = "job_with_predicates_ttj_vs_hj.pdf"
...
def job_speedup(plot_conf: dict):
    ...
    plt.xticks(rotation=270)
    plt.tight_layout()
    filename = "job_speedup_with_predicates_ttj_vs_hj.pdf"
    file_path = construct_fig_name(plot_conf, filename)
    plt.savefig(file_path, format='pdf')
    plt.show()
...
def compute_perecentile_of_improvements(plot_conf: dict):
    """
    We compute the given performance improvement of TTJ compared to the rest three algorithms in JOB with predicates
    """
    raw_data = extract_data_from_csv(Path(plot_conf[DATA_SOURCE_CSV]), column_range=[1, plot_conf[COLUMN_RIGHT_BOUND]])
    speedup_ttj_hj = 'speedup_ttj_hj'

    distribution_of_perform_improve_percentage = dict()
    for algorithm, dps in raw_data.items():
        for query_label, value in dps.items():
            if speedup_ttj_hj not in distribution_of_perform_improve_percentage:
                distribution_of_perform_improve_percentage[speedup_ttj_hj] = []
            distribution_of_perform_improve_percentage[speedup_ttj_hj].append((raw_data[HJ][query_label] - raw_data[TTJ][query_label]) / raw_data[HJ][query_label])
        break

    percentiles = dict()
    for speedup_ttj_algorithm, dps in distribution_of_perform_improve_percentage.items():
        if speedup_ttj_algorithm not in percentiles:
            percentiles[speedup_ttj_algorithm] = dict()
        for percentile in plot_conf[PERCENTILES]:
            percentiles[speedup_ttj_algorithm][percentile] = np.percentile(np.array(dps), percentile)
    print(percentiles)


def compute_statistics(plot_conf: dict):
    raw_data = extract_data_from_csv(Path(plot_conf[DATA_SOURCE_CSV]), column_range=[1, plot_conf[COLUMN_RIGHT_BOUND]])
    speedup_ttj_hj = 'speedup_ttj_hj'
    ...
    stats = dict()
    for speedup_ttj_algorithm, dps in distribution_of_perform_improve_percentage.items():
        if speedup_ttj_algorithm not in stats:
            stats[speedup_ttj_algorithm] = dict()
        stats[speedup_ttj_algorithm]["min"] = np.min(np.array(dps))
        stats[speedup_ttj_algorithm]["max"] = np.max(np.array(dps))
        stats[speedup_ttj_algorithm]["avg"] = np.average(np.array(dps))
    print(stats)
...
if __name__ == "__main__":
    job_plot = {
        ...
        PLOT_FUNCTION: [job_perf_improvements, job_speedup, compute_statistics],
        FIG_SAVE_LOCATION: r".",
