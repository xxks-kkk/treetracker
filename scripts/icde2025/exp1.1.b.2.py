...
Almost identical to exp1.1.b.py; the only exception is we use HJ as the horizontal
line.
"""
...
from icde2025.sqlite_perf import process_job_raw_data
from plot.constants import DATA_SOURCE_CSV, ALGORITHMS_TO_PLOT, HJ, TTJ, Yannakakis, COLUMN_RIGHT_BOUND, YannakakisB, \
    Yannakakis1Pass, SQLITE, TTJ_COLOR, Yannakakis1Pass_COLOR, SQLITE_COLOR, HJ_COLOR
...
def plot_job():
    def bar_plot(ax, data, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        bars = []
        for i, name in enumerate([SQLITE, Yannakakis1Pass, TTJ]):
            values = data[name]
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{SQLite}$', r'$\mathsf{YA}^+$', r'$\mathsf{TTJ}$'], fontsize=8, ncol=3, frameon=False,
                      loc='upper right')
        ...
        font = {'family': 'Helvetica',
                'weight': 'bold',
                'size': 10}
        ax.text(0.00, 1.3, r"$\mathsf{HJ}$", color="#FF0000", transform=trans,
    ...
    prem_data[SQLITE] = process_job_raw_data()
    ...
    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 8}
    bar_plot(axs[0], data_chunk1, data_chunk1_label, colors=[SQLITE_COLOR, Yannakakis1Pass_COLOR, TTJ_COLOR], total_width=.7, single_width=1,
             fontdict=font)
    bar_plot(axs[1], data_chunk2, data_chunk2_label, colors=[SQLITE_COLOR, Yannakakis1Pass_COLOR, TTJ_COLOR], total_width=.7, single_width=1,
    ...
    filename = "exp1.1-bar-job-sqlite-hj-baseline.pdf"
