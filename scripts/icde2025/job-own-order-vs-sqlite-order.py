"""
We plot the TTJHP results on JOB using our own HJ order (shallow join tree) vs. sqlite HJ order (shallow_join_tree)
vs. TTJ order (optimized join tree via DP)

The data comes from sigmod2025/exp1.1.b.py and icde2025/exp2.4.py
"""
...
from plot.constants import DATA_SOURCE_CSV, ALGORITHMS_TO_PLOT, HJ, TTJ, Yannakakis, COLUMN_RIGHT_BOUND, YannakakisB, \
    Yannakakis1Pass, Yannakakis1Pass_COLOR, TTJ_COLOR
from plot.cost_model4 import extract_data_from_csv
from plot.utility import check_argument

DATA_SOURCE_CSV_SQLITE="DATA_SOURCE_CSV_SQLITE"
DATA_SOURCE_CSV_OWN_HJ_ORDER_SHALLOW="DATA_SOURCE_CSV_OWN_HJ_ORDER_SHALLOW"

TTJ_SQLITE="TTJ_SQLITE"
TTJ_OWN_HJ_ORDER="TTJ_OWN_HJ_ORDER"

TTJ_OWN_HJ_ORDER_COLOR=Yannakakis1Pass_COLOR
...
def plot_job():
    def bar_plot(ax, data, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        bars = []
        for i, name in enumerate([TTJ, TTJ_OWN_HJ_ORDER]):
            values = data[name]
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{TTJ}^o$', r'$\mathsf{TTJ}^{HJ}$'], fontsize=10, ncol=3, frameon=False,
                      loc='best')
        ...
        font = {'family': 'Helvetica',
                'weight': 'bold',
                'size': 10}
        ax.text(0.00, 1.3, r"$\mathsf{TTJ}^{SQLite}$", color="#FF0000", transform=trans,
    ...
    job_plot = {
        # TTJ order (optimized join tree via DP)
        DATA_SOURCE_CSV: "2024-03-10T20:18:16.460423Z_perf_report.csv",
        # HJ order from our own optimizer (shallow join tree)
        DATA_SOURCE_CSV_OWN_HJ_ORDER_SHALLOW: "2024-07-01T03:39:00.177389Z_perf_report.csv",
        # HJ order from SQLite (shallow join tree)
        DATA_SOURCE_CSV_SQLITE: "hj_ordering_hj/benchmarkjobwithpredicatesfixedhjorderingshallow-result-2024-07-07t17:19:27.127911_perf_report.csv",
    ...
    data = dict()
    data[TTJ] = prem_data[TTJ]
    prem_data = extract_data_from_csv(get_job_full_path(job_plot[DATA_SOURCE_CSV_OWN_HJ_ORDER_SHALLOW]),
                                      column_range=[1, job_plot[COLUMN_RIGHT_BOUND]])
    data[TTJ_OWN_HJ_ORDER] = prem_data[TTJ]
    prem_data = extract_data_from_csv(get_job_full_path(job_plot[DATA_SOURCE_CSV_SQLITE]),
                                      column_range=[1, job_plot[COLUMN_RIGHT_BOUND]])
    data[TTJ_SQLITE] = prem_data[TTJ]
    ...
    for algorithm, dps in grouped_data.items():
        ttj_sqlite = grouped_data[TTJ_SQLITE]
        data_speedup[algorithm] = \
            [round(ttj_sqlite_time / algorithm_time, 1) for ttj_sqlite_time, algorithm_time in zip(ttj_sqlite, grouped_data[algorithm])]
    del data_speedup[TTJ_SQLITE]
    ...
    fig, axs = plt.subplots(2, 1, figsize=(14, 3), dpi=140)
    ...
    numdp = len(labels)
    chunk_1_endpoint = round(numdp/2)
    data_chunk1, data_chunk2 = dict(), dict()
    data_chunk1_label, data_chunk2_label = labels[:chunk_1_endpoint],labels[chunk_1_endpoint:]
    for algorithms, dps in data_speedup.items():
        data_chunk1[algorithms] = dps[:chunk_1_endpoint]
        data_chunk2[algorithms] = dps[chunk_1_endpoint:]

    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 8}
    bar_plot(axs[0], data_chunk1, data_chunk1_label, colors=[TTJ_COLOR, TTJ_OWN_HJ_ORDER_COLOR], total_width=.7, single_width=1,
             fontdict=font)
    bar_plot(axs[1], data_chunk2, data_chunk2_label, colors=[TTJ_COLOR, TTJ_OWN_HJ_ORDER_COLOR], total_width=.7, single_width=1,
             fontdict=font)
    for ax in axs:
        ax.margins(x=0.01)
        font2 = {'family': 'Helvetica',
                 'weight': 'bold',
                 'size': 10}
        ax.set_ylabel('Speedup', fontdict=font2)
    plt.tight_layout()
    fig.align_ylabels(axs)
    filename = "job-orders-vs-ttj.pdf"
