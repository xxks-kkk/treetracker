...
def process_algorithm(json_data, algorithm):
    def process_ya(json_data):
        join_number_of_hash_table_probe = json_data['Aggregation Stats']['totalIntermediateResultsProducedWithoutNULL']
        summation_of_semijoin_output_size = json_data['Aggregation Stats']['summationOfSemijoinOutputSize']
        return join_number_of_hash_table_probe + summation_of_semijoin_output_size

    def process_hj(json_data):
        return json_data['Aggregation Stats']['totalIntermediateResultsProducedWithoutNULL']
...
def job_speedup():
    ...
    plt.ylabel('Intermediate results ratio (log scale)', fontdict=font2)
    plt.margins(x=0.01)
    plt.tight_layout()
    filename = 'sum-of-intermediate-results.pdf'
