"""
We measure the summation of intermediate result sizes for HJ, YA, and TTJ.
The goal is to explain why YA can run faster than TTJ.
"""
import json
...
from plot.constants import TTJ, HJ, Yannakakis1Pass, DATA_SOURCE_CSV, COLUMN_RIGHT_BOUND, TTJ_COLOR, \
...
def process_algorithm(json_data, algorithm):
    def process_ya(json_data):
        join_number_of_hash_table_probe = json_data['Aggregation Stats']['numberOfHashTableProbe']
        number_of_hash_table_build = json_data['Aggregation Stats']['innerDomainSize'] + json_data['Aggregation Stats']['numberOfHashTableBuildTuplesWithinFullReducer']
        return join_number_of_hash_table_probe + number_of_hash_table_build

    def process_hj(json_data):
        return json_data['Aggregation Stats']['numberOfHashTableProbe'] + json_data['Aggregation Stats']['innerRelationSize']

    def process_ttj(json_data):
        return json_data['Aggregation Stats']['numberOfHashTableProbe'] + json_data['Aggregation Stats']['innerRelationSize']

    check_argument(algorithm in [TTJ, HJ, Yannakakis1Pass], f"invalid algorithm provided: {algorithm}")
    if algorithm == Yannakakis1Pass:
        return process_ya(json_data)
    elif algorithm == HJ:
        return process_hj(json_data)
    else:
        return process_ttj(json_data)


def obtain_query_name(json_data, algorithm):
    def obtain_query_name_raw(raw, algorithm):
        parts = raw.split('.')
        check_argument(algorithm in [TTJ, HJ, Yannakakis1Pass], f"invalid algorithm provided: {algorithm}")
        if algorithm == Yannakakis1Pass:
            suffix = 'OptJoinTreeOptOrderingY1PShallowHJOrdering'
        elif algorithm == HJ:
            suffix = 'FindOptJoinTree'
        else:
            suffix = 'OptJoinTreeOptOrderingShallowHJOrdering'
        beginning_of_suffix = parts[-1].index(suffix)
        ending_of_prefix = parts[-1].index('Query') + len('Query')
        return "query" + parts[-1][ending_of_prefix:beginning_of_suffix]

    return obtain_query_name_raw(json_data['Aggregation Stats']['queryName'], algorithm)

def process():
    result = dict()
    for algorithm in [TTJ, HJ, Yannakakis1Pass]:
        result[algorithm] = dict()
        for stats_file in list(Path(JOB_AGG_STATS_DIR).glob(algorithm+'*.json')):
            with open(stats_file.as_posix(), "r") as fd:
                json_data = json.load(fd)
                check_argument(json_data['Aggregation Stats']['algorithm'] == algorithm,
                               "processing unmatching json")
                result[algorithm][obtain_query_name(json_data,algorithm)] = \
                    process_algorithm(json_data, algorithm)
    return result
...
def job_speedup():
    def bar_plot(ax, data, enable_labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        for i, name in enumerate([TTJ, Yannakakis1Pass]):
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{TTJ}$', r'$\mathsf{YA}^+$'], frameon=False, fontsize=15, ncol=2,
                      loc='best')
        ...
        # ax.set_yticks(list(ax.get_yticks()) + [10])
        # ax.set_ylim([0.1, 8])
    ...
    # CSV for TTJHP and Yannakakis1Pass on fixed HJ ordering
    ...
    for dps in data.values():
        check_argument(len(dps) == job_plot_HJ_ordering[COLUMN_RIGHT_BOUND] - 1,
    ...
    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 8}
    bar_plot(ax, data_speedup, enable_labels=True, colors=[TTJ_COLOR, Yannakakis1Pass_COLOR], total_width=.7, single_width=1,
             fontdict=font)
    font2 = {'family': 'Helvetica',
             'weight': 'bold',
             'size': 10}
    # plt.ylabel('Perf. Improvement %', fontdict=font2)
    plt.ylabel('hash table prob + build ratio (log scale)', fontdict=font2)
    plt.margins(x=0.01)
    plt.tight_layout()
    filename = 'num-of-hash-table-probe-build.pdf'
