...
def plot_job():
    def bar_plot(ax, data, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        for i, name in enumerate([Yannakakis1Pass, HJ, TTJ]):
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{YA}^+$', r"$\mathsf{HJ}$", r'$\mathsf{TTJ}$'], fontsize=8, ncol=3, frameon=False,
                      loc='upper right')
        ...
        font = {'family': 'Helvetica',
                'weight': 'bold',
                'size': 10}
        ax.text(0.015, 1.7, r'$\mathsf{SQLite}$', color="#FF0000", transform=trans,
    ...
    for algorithm, dps in grouped_data.items():
        sqlite = grouped_data[SQLITE]
        data_speedup[algorithm] = \
            [round(sqlite_time / algorithm_time, 1) for sqlite_time, algorithm_time in zip(sqlite, grouped_data[algorithm])]
    ...
    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 8}
    bar_plot(axs[0], data_chunk1, data_chunk1_label, colors=[Yannakakis1Pass_COLOR, HJ_COLOR, TTJ_COLOR], total_width=.7, single_width=1,
             fontdict=font)
    bar_plot(axs[1], data_chunk2, data_chunk2_label, colors=[Yannakakis1Pass_COLOR, HJ_COLOR, TTJ_COLOR], total_width=.7, single_width=1,
    ...
    filename = "exp1.1-bar-job-sqlite.pdf"
