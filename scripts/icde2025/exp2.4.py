"""
TTJ on HJ ordering vs. Yannakakis1Pass on HJ ordering vs. HJ on HJ ordering.
Both TTJ and Yannakakis1Pass share the same join tree.
"""
...
from plot.constants import HJ, COLUMN_RIGHT_BOUND, ALGORITHMS_TO_PLOT, DATA_SOURCE_CSV, TTJ, Yannakakis, \
    TTJ_FIXED_HJ_ORDERING, Yannakakis1Pass, Yannakakis1Pass_FIXED_HJ_ORDERING, TTJ_COLOR, Yannakakis1Pass_COLOR
...
def job_speedup(csv_filename, save_figure_name):
    def bar_plot(ax, data, enable_labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        bars = []
        for i, name in enumerate([TTJ_FIXED_HJ_ORDERING, Yannakakis1Pass_FIXED_HJ_ORDERING]):
            values = data[name]
    ...
    data = dict()
    data[HJ] = prem_data_HJ_ordering[HJ]
    data[TTJ_FIXED_HJ_ORDERING] = prem_data_HJ_ordering[TTJ]
    data[Yannakakis1Pass_FIXED_HJ_ORDERING] = prem_data_HJ_ordering[Yannakakis1Pass]
...
if __name__ == "__main__":
    # HJ ordering from our own optimizer
    job_speedup("2024-07-01T03:39:00.177389Z_perf_report.csv", "exp2.4-job-shallow.pdf")
    # HJ ordering from SQLite
    job_speedup("hj_ordering_hj/benchmarkjobwithpredicatesfixedhjorderingshallow-result-2024-07-07t17:19:27.127911_perf_report.csv", "exp2.4-job-shallow-sqlite.pdf")
