...
from plot.constants import DATA_SOURCE_CSV, HJ, COLUMN_RIGHT_BOUND, ALGORITHMS_TO_PLOT, TTJ, Yannakakis, LIP, \
    YannakakisB, Yannakakis1Pass, TTJ_COLOR, Yannakakis1Pass_COLOR
...
def ssb_speedup_with_predicates():
    ...
    def bar_plot(ax, data, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        if legend:
            ax.legend(bars, [r'$\mathsf{TTJ}$', r'$\mathsf{YA}^+$'], fontsize=15, ncol=2, frameon=False,
                      loc='best')
    ...
    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 15}
    bar_plot(ax, data_speedup, colors=[TTJ_COLOR, Yannakakis1Pass_COLOR], total_width=.7, single_width=1,
             fontdict=font)
