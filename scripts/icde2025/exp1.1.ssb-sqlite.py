...
from icde2025.sqlite_perf import process_ssb_raw_data
from plot.constants import DATA_SOURCE_CSV, HJ, COLUMN_RIGHT_BOUND, ALGORITHMS_TO_PLOT, TTJ, Yannakakis, LIP, \
    YannakakisB, Yannakakis1Pass, TTJ_COLOR, Yannakakis1Pass_COLOR, SQLITE, SQLITE_COLOR
...
def ssb_speedup_with_predicates():
    ssb_plot_with_predicates = {
        DATA_SOURCE_CSV: "hj_ordering_hj/benchmarkssb-result-2024-07-15t23:56:26.1061_perf_report.csv",
    ...
    def bar_plot(ax, data, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        for i, name in enumerate([TTJ, Yannakakis1Pass, SQLITE]):
            values = data[name]
        ...
        if legend:
            ax.legend(bars, [r'$\mathsf{TTJ}$', r'$\mathsf{YA}^+$', r'$\mathsf{SQLite}$'], fontsize=15, ncol=2, frameon=False,
                      loc='best')
        ...
        font = {'family': 'Helvetica',
                'weight': 'bold',
                'size': 20}
        ax.text(0.00, 1.2, r"$\mathsf{HJ}$", color="#FF0000", transform=trans,
    ...
    prem_data[SQLITE] = process_ssb_raw_data()
    ...
    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 15}
    bar_plot(ax, data_speedup, colors=[TTJ_COLOR, Yannakakis1Pass_COLOR, SQLITE_COLOR], total_width=.7, single_width=1,
             fontdict=font)
    ...
    plt.margins(x=0.01)
    plt.tight_layout()
    filename = "exp1.1.ssb-sqlite.pdf"
    plt.savefig(filename, format='pdf')
    plt.show()


if __name__ == "__main__":
    ssb_speedup_with_predicates()