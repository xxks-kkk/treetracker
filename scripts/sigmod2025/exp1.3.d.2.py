...
from plot.constants import DATA_SOURCE_CSV, TTJ, HJ, Yannakakis, LIP, COLUMN_RIGHT_BOUND, ALGORITHMS_TO_PLOT, \
    YannakakisB, TTJ_REDUC_TIME, TTJ_JOIN_TIME, Yannakakis_REDUC_TIME, Yannakakis_JOIN_TIME, YannakakisB_REDUC_TIME, \
    YannakakisB_JOIN_TIME, LIP_REDUC_TIME, LIP_JOIN_TIME, HJ_JOIN_TIME
        ...
        with open(csv_file_path, "r") as file:
            ...
            for row in csv_file:
                if main_sample_count in row:
                    result[algorithm][main_sample_count] = [to_float(num) for num in row[1:]]
                elif join_fragment_eval_count in row:
...
def plot_ssb():
    ...
    for algorithm in agg_data:
        ...
        for i in range(len(agg_data[algorithm][main_sample_count])):
            reduction_time_i = 0
            for time in agg_data[algorithm]:
                if time != main_sample_count and time != join_fragment_eval_count:
    ...
    ssb_plot_with_predicates = {
        DATA_SOURCE_CSV: "2023-12-19T21:37:22.803158Z_perf_report.csv",
    ...
    check_argument(len(prem_data.keys()) == len(ssb_plot_with_predicates[ALGORITHMS_TO_PLOT]),
                   f"there should be {len(ssb_plot_with_predicates[ALGORITHMS_TO_PLOT])} algorithms in data. Current only have {prem_data.keys()}")
    ...
    for algorithm in prem_data.keys():
        if algorithm in ssb_plot_with_predicates[ALGORITHMS_TO_PLOT]:
    ...
    def bar_plot(ax, data, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        bars = []
        for i, name in enumerate([TTJ_REDUC_TIME, TTJ_JOIN_TIME,
                                  Yannakakis_REDUC_TIME, Yannakakis_JOIN_TIME,
                                  YannakakisB_REDUC_TIME, YannakakisB_JOIN_TIME,
                                  LIP_REDUC_TIME, LIP_JOIN_TIME]):
            values = data[name]
        ...
        if legend:
            ax.legend(bars, [r'$\mathsf{TTJ}^R$', r'$\mathsf{TTJ}^\bowtie$',
                             ...
                             r'$\mathsf{PT}^R$', r'$\mathsf{PT}^\bowtie$',
                             r'$\mathsf{LIP}^R$', r'$\mathsf{LIP}^\bowtie$'], frameon=False, fontsize=10, ncol=3,
                      loc='best')
    ...
    for algorithm in preprocessing_time:
        ...
        #     summary[HJ_REDUC_TIME] = preprocessing_time[algorithm]
            ...
            summary[LIP_REDUC_TIME] = preprocessing_time[algorithm]
    for algorithm in join_time:
        if algorithm == TTJ:
            ...
            summary[LIP_JOIN_TIME] = join_time[algorithm]

    bar_plot(ax, summary, labels,
              colors=['#00994D', '#7fcca6', '#FF9933', '#ffcc99', "#00C3E3", "#7fe1f1", '#9933FF', '#cc99ff'], total_width=.7,
              single_width=1,
              fontdict=font,
              legend=True)

    plt.tight_layout()
    filename = "exp1.3.d.2.pdf"
