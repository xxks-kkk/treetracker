...
def plot_ssb():
    ...
    for algorithm in agg_data:
        ...
        for i in range(len(agg_data[algorithm][main_sample_count])):
            if algorithm != TTJ:
                ...
                reduction_time_percentage[algorithm].append(0)
    ...
    def bar_plot(ax, data, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        bars = []
        for i, name in enumerate([HJ, TTJ, Yannakakis, YannakakisB, LIP]):
            values = data[name]
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{HJ}$', r'$\mathsf{TTJ}$', r'Yannakakis', r'$\mathsf{LIP}$'], frameon=False, fontsize=10, ncol=3,
                      loc='best')
    ...
    def bar_plot2(ax, data, reduction_time_percentage_use, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        for i, name in enumerate([HJ, TTJ, Yannakakis, YannakakisB, LIP]):
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{HJ}^\bowtie$', r'$\mathsf{TTJ}^\bowtie$', r'$\mathsf{YA}^\bowtie$', r'$\mathsf{PT}^\bowtie$', r'$\mathsf{LIP}^\bowtie$'], frameon=False, fontsize=10, ncol=3,
                      loc='best')
        x = np.arange(len(labels))
        if fontdict is not None:
            ax.set_xticks(x)
    ...
    font = {'family': 'Helvetica',
            'size': 10}
    bar_plot(ax, data, labels=labels, colors=["#FF0000", '#00994D', '#FF9933', "#00C3E3", '#9933FF'], total_width=.7,
             single_width=1,
             fontdict=font,
             legend=False)
    ...
    bar_plot2(ax, join_time, reduction_time_percentage, labels=labels, colors=['#ff7f7f', '#7fcca6', '#ffcc99', "#7fe1f1", '#cc99ff'], total_width=.7,
             single_width=1,
             fontdict=font,
             legend=True)

    plt.tight_layout()
    filename = "exp1.3.d.pdf"
