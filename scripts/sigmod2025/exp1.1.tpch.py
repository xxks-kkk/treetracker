...
from plot.constants import DATA_SOURCE_CSV, DECIMAL_PRECISION, HJ, PLOT_FUNCTION, FIG_SAVE_LOCATION, \
    SORT_DESCENDING_BASED_ON_HJ, COLUMN_RIGHT_BOUND, ALGORITHMS_TO_PLOT, TTJ, Yannakakis, YannakakisB, YannakakisV, PTO
...
def tpch_speedup_with_predicates():
    tpch_plot_with_predicates = {
        DATA_SOURCE_CSV: "2023-12-23T20:18:11.143893Z_perf_report.csv",
        ALGORITHMS_TO_PLOT: [HJ, TTJ, Yannakakis, YannakakisB, YannakakisV, PTO],
    ...
    def bar_plot(ax, data, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        for i, name in enumerate([TTJ, Yannakakis, YannakakisV, YannakakisB, PTO]):
            values = data[name]
        ...
        if legend:
            ax.legend(bars, [r'$\mathsf{TTJ}$', r'$\mathsf{YA}$', r'$\mathsf{YAV}$', r'$\mathsf{PT}$', r'$\mathsf{PTO}$'], fontsize=15, ncol=2, frameon=False,
                      loc='best')
    ...
    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 15}
    bar_plot(ax, data_speedup, colors=['#00994D', '#FF9933', '#9933FF', "#00C3E3", "#F1D766"], total_width=.7, single_width=1,
             fontdict=font)
