...
def job_improvement_percentage():
    def bar_plot(ax, data, enable_labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        for i, (name, values) in enumerate(data.items()):
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{TTJ}$', r'Yannakakis'], frameon=False, fontsize=15, ncol=1,
                      loc='best')
        ...
        font = {'family': 'Helvetica',
                'weight': 'bold',
                'size': 20}
        ax.text(0.05, 0.12, r"$\mathsf{HJ}$", color="#FF0000", transform=trans,
    ...
    for algorithm, dps in grouped_data.items():
        ...
        data_speedup[algorithm] = \
            [(hj_time - algorithm_time)/float(hj_time) for hj_time, algorithm_time in zip(hj, grouped_data[algorithm])]
    del data_speedup[HJ]
    ...
    plt.tight_layout()
    filename = "exp1.1-d-job.pdf"
    plt.savefig(filename, format='pdf')
    plt.show()

if __name__ == "__main__":
    job_improvement_percentage()