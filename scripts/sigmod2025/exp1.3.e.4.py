...
def plot_tpch():
    ...
    tpch_plot_with_predicates = {
        DATA_SOURCE_CSV: "2024-01-10T20:44:11.729653Z_perf_report.csv",
    ...
    def bar_plot2(ax, data, abels, colors=None, total_width=0.8, single_width=1,
        ...
        for i, name in enumerate([HJ, TTJ, Yannakakis, YannakakisB]):
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{HJ}$', r'$\mathsf{TTJ}$', r'$\mathsf{YA}$', r'$\mathsf{PT}$'], frameon=False, fontsize=10, ncol=2,
                      loc='best')
        x = np.arange(len(labels))
        if fontdict is not None:
            ax.set_xticks(x)
    ...
    bar_plot2(ax, data, labels,
              colors=["#FF0000", '#00994D', '#7fcca6', '#FF9933', '#ffcc99', "#00C3E3", "#7fe1f1"], total_width=.7,
              single_width=1,
              fontdict=font,
              legend=True)

    plt.tight_layout()
    filename = "exp1.3.e.4.pdf"
