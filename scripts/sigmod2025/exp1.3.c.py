...
def plot_ssb():
    ...
    def bar_plot2(ax, data, reduction_time_percentage_use, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        for i, name in enumerate([TTJ, Yannakakis, YannakakisB, LIP]):
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{TTJ}^\bowtie$', r'$\mathsf{YA}^\bowtie$', r'$\mathsf{PT}^\bowtie$', r'$\mathsf{LIP}^\bowtie$'], frameon=False, fontsize=10, ncol=2,
                      loc='best')
        x = np.arange(len(labels))
        if fontdict is not None:
            ax.set_xticks(x)
    ...
    font = {'family': 'Helvetica',
            'size': 10}
    bar_plot(ax, data, labels=labels, colors=['#00994D', '#FF9933', "#00C3E3", '#9933FF'], total_width=.7,
             single_width=1,
             fontdict=font,
             legend=False)
    ...
    bar_plot2(ax, join_time, reduction_time_percentage, labels=labels, colors=['#7fcca6', '#ffcc99', "#7fe1f1", '#cc99ff'], total_width=.7,
             single_width=1,
             fontdict=font,
             legend=True)

    plt.tight_layout()
    filename = "exp1.3.c.pdf"
