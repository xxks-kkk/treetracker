...
def job_speedup():
    def bar_plot(ax, data, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        for i, (name, values) in enumerate(data.items()):
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{TTJ}^{bf}$', r'$\mathsf{TTJ}^{bf+df}$', r'$\mathsf{TTJ}$'], frameon=False,
    ...
    job_plot = {
        DATA_SOURCE_CSV: "2023-11-22T21:41:26.957516Z_perf_report.csv",
        ALGORITHMS_TO_PLOT: [HJ, TTJ, TTJ_NO_NG, TTJ_BF],
    ...
    def analyze_data_speedup():
        ...
        bd_useful = []
        nf_useful = []
        for i, (hj_dp, bf_dp, bd_dp, ttj_dp) in enumerate(
                zip(data_speedup[HJ], data_speedup[TTJ_BF], data_speedup[TTJ_NO_NG], data_speedup[TTJ])):
            if bf_dp > hj_dp:
        ...
        labels_to_print = []
        candidate_labels = bf_useful + bd_useful + nf_useful
        for label in labels:
    ...
    del data_speedup[HJ]
    for algorithm in [TTJ_BF, TTJ_NO_NG, TTJ]:
        data_speedup_use[algorithm] = data_speedup[algorithm]
    ...
    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 7}
    bar_plot(ax, data_speedup_use, labels=labels_to_print, colors=['#FF9933', '#9933FF', '#00994D'], total_width=.7,
             single_width=1,
             fontdict=font)
