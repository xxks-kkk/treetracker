...
def plot_tpch():
    ...
    for algorithm in agg_data:
        ...
        for i in range(len(agg_data[algorithm][join_fragment_eval_count])):
            ...
            for time in agg_data[algorithm]:
                if time != join_fragment_eval_count:
    ...
    def bar_plot(ax, data, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        bars = []
        for i, name in enumerate([HJ, TTJ, Yannakakis, YannakakisB]):
            values = data[name]
    ...
    def bar_plot2(ax, data, reduction_time_percentage_use, labels, colors=None, total_width=0.8, single_width=1,
        ...
        for i, name in enumerate([HJ, TTJ, Yannakakis, YannakakisB]):
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{HJ}^\bowtie$', r'$\mathsf{TTJ}^\bowtie$', r'$\mathsf{YA}^\bowtie$',
                             r'$\mathsf{PT}^\bowtie$'], frameon=False, fontsize=10, ncol=3,
                      loc='best')
        x = np.arange(len(labels))
        if fontdict is not None:
            ax.set_xticks(x)
    ...
    bar_plot2(ax, join_time, reduction_time_percentage, labels=labels,
              colors=['#ff7f7f', '#7fcca6', '#ffcc99', "#7fe1f1"], total_width=.7,
              single_width=1,
              fontdict=font,
              legend=True)

    plt.tight_layout()
    filename = "exp1.3.tpch.pdf"
