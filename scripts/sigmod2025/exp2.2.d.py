...
def plot_ssb3(ratio, filename, ylabel):
    ymax = 1.1
        ...
        for i, name in enumerate([TTJ, Yannakakis, YannakakisB]):
            ...
            for x, y in enumerate(values):
                ...
                if rect.get_height() > 1:
                    # ax.bar_label(bar, fmt=f'{rect.get_height():.2f}', rotation='vertical', xytext=(rect.get_x(), rect.get_height()))
                    ax.annotate(f'{rect.get_height():.0%}', (rect.get_x() + rect.get_width()/2, min(rect.get_height(),ymax)),
                                fontproperties={'family': 'Helvetica',
                                                'size': 8}, ha='center', va='bottom',
                                rotation=70)
                    # ax.bar_laeb(rect.get_x(), rect.get_height(), f'{rect.get_height():.2f}', ha='center', va='bottom',
                    #         rotation='vertical')
                bar_list.append(bar)
            bars.append(bar[0])
            # for rect in [ibar[0] for ibar in bar_list]:
...
if __name__ == "__main__":
    ...
    plot_ssb3(intermediate_ratio, "exp2.2.d.1.pdf", r'Fraction of $\mathsf{HJ}$ intermediate result size')
    plot_ssb(input_size_ratio, "exp2.2.d.2.pdf", r'Fraction of input tuples remained')
    plot_ssb2(tuple_removed_ratio, "exp2.2.d.3.pdf", 'Fraction of input tuples removed')