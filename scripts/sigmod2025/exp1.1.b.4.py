...
def plot_job():
    def bar_plot(ax, data, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        for i, name in enumerate([TTJ, Yannakakis, Yannakakis1Pass, YannakakisB]):
            ...
            bars.append(bar[0])
        if legend:
            ax.legend(bars, [r'$\mathsf{TTJ}$', r'$\mathsf{YA}$', r'$\mathsf{YA}^+$', r'$\mathsf{PT}$'], fontsize=10, ncol=4, frameon=False,
                      loc='best')
    ...
    fig, axs = plt.subplots(3, 1, figsize=(14, 3), dpi=140)
    ...
    bar_plot(axs[2], data_chunk3, data_chunk3_label, colors=['#00994D', '#FF9933', '#EA2283', "#00C3E3"], total_width=.7, single_width=1,
             fontdict=font, legend=False)
    for ax in axs:
        ax.margins(x=0.01)
