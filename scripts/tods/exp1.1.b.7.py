...
from plot.constants import DATA_SOURCE_CSV, ALGORITHMS_TO_PLOT, HJ, TTJ, COLUMN_RIGHT_BOUND, Yannakakis1Pass, TTJ_COLOR, \
    Yannakakis1Pass_COLOR, HJ_COLOR, POSTGRES, JOB_SQLITE_ORDERING_RESULTS_INTROW_ON
from plot.cost_model4 import extract_data_from_csv
from plot.utility import check_argument

# Suppress findFont warning (https://stackoverflow.com/a/58393562/1460102)
...
def ttj_perf_analysis(grouped_data, labels):
    ...
    yannakakis1Pass_faster_TTJ = []
    for idx in range(len(grouped_data[TTJ])):
        if grouped_data[TTJ][idx] > grouped_data[Yannakakis1Pass][idx]:
            yannakakis1Pass_faster_TTJ.append(labels[idx])

    print(f"Yannakakis1Pass is faster than TTJ in {yannakakis1Pass_faster_TTJ}")
...
def plot_job():
    def bar_plot(ax, data, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None,
        ...
        if legend:
            ...
            ax.legend(bars, [r'$\mathsf{TTJ}$', r"$\mathsf{HJ}$", r'$\mathsf{YA}$', 'Postgres'], fontsize=13, ncol=4,
                      frameon=False,
    ...
    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 10}
    bar_plot(axs[0], data_chunk1, data_chunk1_label, colors=[TTJ_COLOR, HJ_COLOR, Yannakakis1Pass_COLOR],
             total_width=.7, single_width=1,
             fontdict=font, legend=True)
    bar_plot(axs[1], data_chunk2, data_chunk2_label, colors=[TTJ_COLOR, HJ_COLOR, Yannakakis1Pass_COLOR],
             total_width=.7, single_width=1,
             fontdict=font, legend=False)
    bar_plot(axs[2], data_chunk3, data_chunk3_label, colors=[TTJ_COLOR, HJ_COLOR, Yannakakis1Pass_COLOR],
             total_width=.7, single_width=1,
             fontdict=font, legend=False)
    bar_plot(axs[3], data_chunk4, data_chunk4_label, colors=[TTJ_COLOR, HJ_COLOR, Yannakakis1Pass_COLOR],
             total_width=.7, single_width=1,
             fontdict=font, legend=False)
    ...
    filename = "exp1.1-bar-job-postgres-7.pdf"
