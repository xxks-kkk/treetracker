...
from plot.constants import DATA_SOURCE_CSV, HJ, COLUMN_RIGHT_BOUND, ALGORITHMS_TO_PLOT, TTJ, Yannakakis1Pass, TTJ_COLOR, \
    Yannakakis1Pass_COLOR, SQLITE, HJ_COLOR, TTJ_NO_NG, TTJ_NO_DP, \
    TTJ_VANILLA, TTJ_NO_DP_COLOR, TTJ_NO_NG_COLOR, TTJ_VANILLA_COLOR, POSTGRES
...
def tpch_speedup_with_predicates():
    ...
    def bar_plot(ax, data, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None):
        ...
        if legend:
            ...
            ax.legend(bars, [r'$\mathsf{TTJ}^{dp+ng}$', r'$\mathsf{TTJ}^{ng}$', r'$\mathsf{TTJ}^{dp}$', r'$\mathsf{TTJ}$', r"$\mathsf{HJ}$", r'$\mathsf{YA}$', 'Postgres'], fontsize=15, ncol=5, frameon=False,
    ...
    for algorithm, dps in data.items():
        ...
        data_speedup[algorithm] = \
            [round(postgres_time / algorithm_time, 1) for postgres_time, algorithm_time in zip(postgres, data[algorithm])]
    ...
    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 15}
    bar_plot(ax, data_speedup, colors=[TTJ_COLOR, TTJ_NO_DP_COLOR, TTJ_NO_NG_COLOR, TTJ_VANILLA_COLOR, HJ_COLOR, Yannakakis1Pass_COLOR], total_width=.7, single_width=1,
             fontdict=font)
    ...
    plt.margins(x=0.01)
    plt.tight_layout()
    filename = "tpch-ablation-postgres.pdf"
