"""
Performance results on Postgres plans with IntRow optimization disabled.
The script compares TTJ performance under bushy plan with TTJ under linear plan.
"""
import logging
...
from plot.constants import DATA_SOURCE_CSV, ALGORITHMS_TO_PLOT, TTJ, COLUMN_RIGHT_BOUND, TTJ_COLOR, \
    Yannakakis1Pass_COLOR, HJ_COLOR, POSTGRES, TTJ_LINEAR, JOB_SQLITE_ORDERING_RESULTS_INTROW_ON, HJ, TTJ_NO_NG_COLOR, \
    JOB_POSTGRES_ORDERGING_RESULTS_INTROW_OFF
...
def ttj_perf_analysis(ttj_speedup, ttj_linear_speedup, labels):
    count_ttj_fastest = 0
    total_queries = len(ttj_speedup)

    for idx in range(len(ttj_speedup)):
...
def ttj_speedup_analysis(TTJ_time, baseline, baseline_algorithm, labels):
    print(f"Speedup analysis for TTJ over {baseline_algorithm}")
    data_speedup = dict()
    data_speedup[TTJ] = \
        [round(baseline_time / algorithm_time, 1) for baseline_time, algorithm_time in zip(baseline, TTJ_time)]
...
def plot_job():
    def bar_plot(ax, data, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None,
        ...
        bars = []
        for i, name in enumerate([TTJ, TTJ_LINEAR, HJ]):
            values = data[name]
        ...
        if legend:
            ...
            ax.legend(bars, [r'$\mathsf{TTJ}$', r'$\mathsf{TTJ}^{L}$', r'$\mathsf{HJ}$', 'Postgres'], fontsize=13,
    ...
    job_plot = {
        ...
        ALGORITHMS_TO_PLOT: [TTJ, HJ, POSTGRES],
        COLUMN_RIGHT_BOUND: 114
    }
    ...
    for algorithm, dps in grouped_data.items():
        ...
        data_speedup[algorithm] = \
            [round(postgres_time / algorithm_time, 1) for postgres_time, algorithm_time in
             zip(postgres, grouped_data[algorithm])]
    del data_speedup[POSTGRES]

    for algorithm, dps in grouped_data_linear.items():
        postgres = grouped_data[POSTGRES]
        if algorithm == TTJ:
            data_speedup[TTJ_LINEAR] = \
                [round(postgres_time / algorithm_time, 1) for postgres_time, algorithm_time in
                 zip(postgres, grouped_data_linear[algorithm])]

    speedup_analysis(data_speedup, labels)
    print_speedup(data_speedup, labels)
    ttj_perf_analysis(data_speedup[TTJ], data_speedup[TTJ_LINEAR], labels)
    ttj_speedup_analysis(grouped_data[TTJ], grouped_data_linear[TTJ], TTJ_LINEAR, labels)
    ttj_speedup_analysis(grouped_data[TTJ], grouped_data[HJ], HJ, labels)

    matplotlib.rcParams['mathtext.fontset'] = 'stix'
    fig, axs = plt.subplots(4, 1, figsize=(14, 6), dpi=300)

    numdp = len(labels)
    chunk_1_endpoint = round(numdp / 4)
    chunk_2_endpoint = chunk_1_endpoint + round(numdp / 4)
    chunk_3_endpoint = chunk_2_endpoint + round(numdp / 4)
    data_chunk1, data_chunk2, data_chunk3, data_chunk4 = dict(), dict(), dict(), dict()
    data_chunk1_label = labels[:chunk_1_endpoint]
    data_chunk2_label = labels[chunk_1_endpoint:chunk_2_endpoint]
    data_chunk3_label = labels[chunk_2_endpoint:chunk_3_endpoint]
    data_chunk4_label = labels[chunk_3_endpoint:]
    ...
    for algorithms, dps in data_speedup.items():
        ...
        data_chunk3[algorithms] = dps[chunk_2_endpoint:chunk_3_endpoint]
        data_chunk4[algorithms] = dps[chunk_3_endpoint:]

    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 10}
    bar_plot(axs[0], data_chunk1, data_chunk1_label,
             colors=[TTJ_COLOR, TTJ_NO_NG_COLOR, HJ_COLOR, Yannakakis1Pass_COLOR],
             total_width=.7, single_width=1,
             fontdict=font, legend=True)
    bar_plot(axs[1], data_chunk2, data_chunk2_label,
             colors=[TTJ_COLOR, TTJ_NO_NG_COLOR, HJ_COLOR, Yannakakis1Pass_COLOR],
             total_width=.7, single_width=1,
             fontdict=font, legend=False)
    bar_plot(axs[2], data_chunk3, data_chunk3_label,
             colors=[TTJ_COLOR, TTJ_NO_NG_COLOR, HJ_COLOR, Yannakakis1Pass_COLOR],
             total_width=.7, single_width=1,
             fontdict=font, legend=False)
    bar_plot(axs[3], data_chunk4, data_chunk4_label,
             colors=[TTJ_COLOR, TTJ_NO_NG_COLOR, HJ_COLOR, Yannakakis1Pass_COLOR],
             total_width=.7, single_width=1,
             fontdict=font, legend=False)
