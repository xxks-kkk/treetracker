...
from plot.constants import DATA_SOURCE_CSV, ALGORITHMS_TO_PLOT, HJ, TTJ, COLUMN_RIGHT_BOUND, Yannakakis1Pass, TTJ_COLOR, \
    ...
    TTJ_VANILLA_COLOR, TTJ_LINEAR, JOB_SQLITE_ORDERING_RESULTS_INTROW_ON, JOB_POSTGRES_ORDERGING_RESULTS_INTROW_OFF
...
def ttj_speedup_analysis(TTJ_time, TTJ_linear_time, labels):
    print(f"Speedup analysis for TTJ over {TTJ_LINEAR}")
    data_speedup = dict()
    baseline = TTJ_linear_time
    data_speedup[TTJ] = \
...
def plot_job():
    def bar_plot(ax, data, labels, colors=None, total_width=0.8, single_width=1, legend=True, fontdict=None,
        ...
        bars = []
        for i, name in enumerate([TTJ, TTJ_LINEAR]):
            values = data[name]
        ...
        if legend:
            ...
            ax.legend(bars, [r'$\mathsf{TTJ}$', r'$\mathsf{TTJ}^{L}$', 'Postgres'], fontsize=13, ncol=6, frameon=False,
    ...
    job_plot = {
        ...
        ALGORITHMS_TO_PLOT: [TTJ, POSTGRES],
        COLUMN_RIGHT_BOUND: 114
    }
    ...
    for algorithm, dps in grouped_data_linear.items():
        ...
        if algorithm == TTJ:
            data_speedup[TTJ_LINEAR] = \
                [round(postgres_time / algorithm_time, 1) for postgres_time, algorithm_time in zip(postgres, grouped_data_linear[algorithm])]

    speedup_analysis(data_speedup, labels)
    print_speedup(data_speedup, labels)
    ttj_perf_analysis(data_speedup[TTJ], data_speedup[TTJ_LINEAR], labels)
    ttj_speedup_analysis(grouped_data[TTJ], grouped_data_linear[TTJ], labels)
    ...
    font = {'family': 'Helvetica',
            'weight': 'bold',
            'size': 10}
    bar_plot(axs[0], data_chunk1, data_chunk1_label, colors=[TTJ_COLOR, HJ_COLOR], total_width=.7, single_width=1,
             fontdict=font)
    bar_plot(axs[1], data_chunk2, data_chunk2_label, colors=[TTJ_COLOR, HJ_COLOR], total_width=.7, single_width=1,
             fontdict=font, legend=False)
    bar_plot(axs[2], data_chunk3, data_chunk3_label, colors=[TTJ_COLOR, HJ_COLOR], total_width=.7, single_width=1,
             fontdict=font, legend=False)
